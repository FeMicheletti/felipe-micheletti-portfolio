      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
            Crie e mantenha seus cases em português e inglês sem alterar o
            código.
          </p>
        </div>
        <Button
          asChild
          className="bg-violet-600 text-white shadow-lg shadow-violet-950/30 hover:bg-violet-500"
        >
        ...
        <div className="overflow-hidden rounded-xl border border-violet-500/10 bg-zinc-900/70 shadow-lg shadow-black/10">
          ...
          <div className="divide-y divide-white/5">
            {projects.map((project) => {
              const pt = project.translations.find(
                (translation) => translation.locale === "PT_BR",
              );
              const en = project.translations.find(
                (translation) => translation.locale === "EN_US",
              );
              ...
              return (
                <div
                  key={project.id}
                  className="grid gap-4 px-4 py-5 md:grid-cols-[minmax(0,1fr)_8rem_7rem_7rem_6rem] md:items-center md:px-5"
                >
                  ...
                  <div className="flex items-center justify-end gap-1">
                    <Button
                      ...
                      variant="ghost"
                      size="icon-sm"
                      className="text-zinc-400 hover:bg-violet-500/10 hover:text-violet-300"
                    >
        ...
        <Card className="border-dashed border-violet-500/20 bg-violet-500/2.5 py-14 text-center ring-0">
          <CardContent className="flex flex-col items-center">
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-zinc-500">
              Comece pelo seu case mais importante. Depois vincularemos stacks e
              imagens.
            </p>
            <Button
              asChild
              variant="ghost"
              className="mt-4 text-violet-300 hover:bg-violet-500/10 hover:text-violet-200"
            >
              <Link href="/admin/projetos/novo">
