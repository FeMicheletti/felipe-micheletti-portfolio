...
export async function uploadMediaAction(
  ...
  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    return { error: "Envie uma imagem JPG, PNG, WebP, GIF, AVIF ou um PDF." };
  }

  if (file.size > MAX_FILE_SIZE) {
    return { error: "O arquivo deve ter no máximo 20 MB." };
  }
  ...
  try {
    ...
    if (uploadedPath) {
      await deleteFromOneDrive(uploadedPath).catch(() => undefined);
    }
...
export async function deleteMediaAction(mediaId: string) {
  ...
  const references =
    media._count.projectMedia +
    media._count.resumes +
    media._count.heroSettings;
