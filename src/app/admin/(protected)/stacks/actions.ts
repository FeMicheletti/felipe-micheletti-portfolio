...
export async function createCategoryAction(_state: StackFormState, formData: FormData): Promise<StackFormState> {
	...
	if (!result.success) return { error: "Revise os campos destacados.", fieldErrors: result.error.flatten().fieldErrors};
	if (await slugInUse("category", result.data.slug)) return { error: "Já existe uma categoria com esse slug.", fieldErrors: { slug: ["Escolha outro slug."] }};
...
export async function updateCategoryAction(categoryId: string, _state: StackFormState, formData: FormData): Promise<StackFormState> {
	...
	if (!result.success) return {error: "Revise os campos destacados.", fieldErrors: result.error.flatten().fieldErrors};
	if (await slugInUse("category", result.data.slug, categoryId)) return {error: "Já existe uma categoria com esse slug.", fieldErrors: { slug: ["Escolha outro slug."] }};
...
export async function createTechnologyAction(_state: StackFormState, formData: FormData): Promise<StackFormState> {
	...
	if (!result.success) return {error: "Revise os campos destacados.", fieldErrors: result.error.flatten().fieldErrors};
	if (await slugInUse("technology", result.data.slug)) return { error: "Já existe uma tecnologia com esse slug.", fieldErrors: { slug: ["Escolha outro slug."] }};
...
export async function updateTechnologyAction(technologyId: string, _state: StackFormState, formData: FormData): Promise<StackFormState> {
	...
	if (!result.success) return { error: "Revise os campos destacados.", fieldErrors: result.error.flatten().fieldErrors};
	if (await slugInUse("technology", result.data.slug, technologyId)) return {error: "Já existe uma tecnologia com esse slug.", fieldErrors: { slug: ["Escolha outro slug."] }};
