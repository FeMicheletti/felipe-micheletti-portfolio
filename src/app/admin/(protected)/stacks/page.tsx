import { AlertCircle, Eye, EyeOff, Layers3, PackagePlus, Pencil, Sparkles } from "lucide-react";
...
const errorMessages: Record<string, string> = {
	"category-has-technologies":
	"A categoria não pode ser excluída enquanto possuir tecnologias.",
	"technology-has-projects":
	"A tecnologia não pode ser excluída enquanto estiver vinculada a projetos.",
};
	...
	return (
		<div className="space-y-6">
			...
			<section className="grid gap-3 sm:grid-cols-3">
				{[{ label: "Categorias", value: categories.length }, { label: "Tecnologias", value: technologies.length }, { label: "Stacks visíveis", value: technologies.filter((item) => item.visible).length }].map((item) => (
								...
								<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
									<div>
										<div className="flex flex-wrap items-center gap-2">
											...
											<Badge className={category.visible ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-zinc-500/20 bg-zinc-500/10 text-zinc-400"}>
												{category.visible ? "Visível" : "Oculta"}
											</Badge>
										</div>
										<CardDescription className="mt-1 text-zinc-500">
											{category.nameEn} · {category.slug} · ordem{" "}
											{category.sortOrder}
										</CardDescription>
									</div>
										...
										<form action={toggleCategoryVisibilityAction.bind( null, category.id, !category.visible )}>
											<Button type="submit" variant="ghost" size="icon-sm" className="text-zinc-500 hover:bg-violet-500/10 hover:text-violet-300">
												...
												<span className="sr-only">
													{category.visible ? "Ocultar" : "Exibir"}{" "}
													{category.namePt}
												</span>
											</Button>
										</form>
											...
											<div key={technology.id} className="p-4">
												<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
													<div className="flex min-w-0 items-center gap-3">
														<span className="size-3 shrink-0 rounded-full ring-2 ring-white/10" style={{ backgroundColor: technology.color ?? "#71717a" }}/>
													...
													<div className="flex items-center justify-end gap-1">
														<Badge className={technology.visible ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-zinc-500/20 bg-zinc-500/10 text-zinc-400"}>
														...
														<form action={toggleTechnologyVisibilityAction.bind( null, technology.id, !technology.visible )}>
															<Button type="submit" variant="ghost" size="icon-sm" className="text-zinc-500 hover:bg-violet-500/10 hover:text-violet-300">
																...
																<span className="sr-only">
																	{technology.visible ? "Ocultar" : "Exibir"}{" "}
																	{technology.name}
																</span>
															</Button>
														</form>
					...
					<Card className="border-dashed border-violet-500/20 bg-violet-500/2.5 py-14 text-center ring-0">
						<CardContent>
							...
							<p className="mt-2 text-sm text-zinc-500">
								Comece criando uma categoria acima.
							</p>
						</CardContent>
					</Card>
				)}
