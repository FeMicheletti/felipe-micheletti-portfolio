...
function oneDriveConfig() {
  ...
  if (!apiUrl || !apiToken) {
    throw new Error("A integração com o OneDrive ainda não foi configurada.");
  }
...
async function responseError(response: Response) {
  try {
    ...
    return (
      body.message || `A API OneDrive respondeu com status ${response.status}.`
    );
  } catch {
...
export async function uploadToOneDrive({
  ...
  if (
    ...
    throw new Error(
      "A API OneDrive retornou dados incompletos para o arquivo.",
    );
  }
...
export async function deleteFromOneDrive(path: string) {
  ...
  const response = await fetch(
    `${apiUrl}/api/v1/files?path=${encodeURIComponent(path)}`,
    {
      method: "DELETE",
      headers: { "X-API-Key": apiToken },
      cache: "no-store",
