...
import { BarChart3, BriefcaseBusiness, Code2, Database, ExternalLink, Gauge, Image, Layers3, LogOut, Menu, Settings, X } from "lucide-react";
...
export function AdminShell({ admin, children }: AdminShellProps) {
	...
	const pageTitle = pathname === "/admin/projetos/novo"
		? "Novo projeto"
		...
		<div className="min-h-screen bg-zinc-950 text-zinc-100">
			...
			<aside className={cn("fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-violet-500/15 bg-zinc-950/95 shadow-2xl shadow-violet-950/20 backdrop-blur-xl transition-transform duration-200 lg:translate-x-0", mobileMenuOpen ? "translate-x-0" : "-translate-x-full")}>
				...
				<nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
					<p className="mb-3 px-3 text-[11px] font-medium tracking-[0.18em] text-zinc-600 uppercase">
						Gerenciamento
					</p>
					...
					{navigation.map((item) => {
						...
						return (
							<Link
								...
								className={cn("flex h-10 items-center gap-3 rounded-lg px-3 text-sm font-medium transition-colors", active ? "bg-violet-600 text-white shadow-lg shadow-violet-950/30" : "text-zinc-400 hover:bg-white/5 hover:text-zinc-100")}>
								<Icon className="size-4" />
								{item.label}
							</Link>
						);
