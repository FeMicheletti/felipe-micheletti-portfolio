...
export function ProjectMediaFields({
	...
	function setCover(mediaId: string) {
		const withoutCover = value.filter(
			(item) => item.role !== "COVER" && item.mediaId !== mediaId,
		);
	...
	function toggleGallery(mediaId: string, checked: boolean) {
		...
		if (
			...
			value.some((item) => item.mediaId === mediaId)
		) {
			return;
		}
		...
		<Card className="border-violet-500/10 bg-zinc-900/70 shadow-lg shadow-black/10 ring-0">
			<CardHeader className="border-b border-white/5">
				...
				<CardDescription className="text-zinc-500">
					Escolha a capa, monte a galeria e descreva as imagens nos dois
					idiomas.
				</CardDescription>
			</CardHeader>
					...
					<>
						<div className="space-y-3">
							...
							{cover && assetById.get(cover.mediaId) ? (
								<div className="grid gap-4 rounded-xl border border-violet-500/10 bg-violet-500/2.5 p-3 sm:grid-cols-[12rem_minmax(0,1fr)]">
									...
									<div className="grid content-start gap-3">
										<div className="space-y-1.5">
											...
											<Input
												...
												maxLength={255}
												onChange={(event) =>
													updateItem(cover.mediaId, "altPt", event.target.value)
												}
										...
										<div className="space-y-1.5">
											...
											<Input
												...
												maxLength={255}
												onChange={(event) =>
													updateItem(cover.mediaId, "altEn", event.target.value)
												}
												className="border-white/10 bg-zinc-950/50"
											/>
										</div>
							...
							<div className="grid gap-2 sm:grid-cols-2">
								{mediaAssets.map((asset) => {
									...
									return (
										<label
											key={asset.id}
											className="flex cursor-pointer items-center gap-3 rounded-lg border border-white/5 bg-zinc-950/30 px-3 py-2.5 text-sm text-zinc-300 transition-colors hover:border-violet-500/20 hover:bg-violet-500/5 has-disabled:cursor-not-allowed has-disabled:opacity-50"
										>
											<input
												type="checkbox"
												checked={selected}
												disabled={isCover}
												onChange={(event) =>
													toggleGallery(asset.id, event.target.checked)
												}
								...
								<div className="space-y-3">
									{gallery.map((item, index) => {
										...
										return (
											<div
												key={item.mediaId}
												className="grid gap-3 rounded-xl border border-white/5 bg-white/2.5 p-3 sm:grid-cols-[8rem_minmax(0,1fr)]"
											>
