...
} from "@/app/admin/(protected)/stacks/actions";
...
} from "@/lib/stacks/stack-form";

const initialState: StackFormState = {};
const controlClass =
  "border-white/10 bg-zinc-950/50 focus-visible:border-violet-500/60 focus-visible:ring-violet-500/20";
    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor={`category-namePt-${id ?? "new"}`}>
            Nome em português
          </Label>
          <Input
        ...
        <div className="space-y-2">
          <Label htmlFor={`category-nameEn-${id ?? "new"}`}>
            Nome em inglês
          </Label>
          <Input
...
export function TechnologyForm({
  ...
  const action = id
    ? updateTechnologyAction.bind(null, id)
    : createTechnologyAction;
  const [state, formAction, pending] = useActionState(action, initialState);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_8rem_7rem]">
        <div className="space-y-2">
          <Label htmlFor={`technology-icon-${id ?? "new"}`}>
            Chave do ícone
          </Label>
          <Input
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor={`technology-descriptionPt-${id ?? "new"}`}>
        ...
        <div className="space-y-2">
          <Label htmlFor={`technology-descriptionEn-${id ?? "new"}`}>
            Descrição em inglês
          </Label>
          <Textarea
